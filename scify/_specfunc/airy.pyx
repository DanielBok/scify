...
cdef double airy_bie(double x) nogil:
    ...
    if x < 4:
        ...
        return (0.625 + cheb_eval_mode(bip_cs, z, 24, -1, 1)) / y
    else:
        z = 16 / (x * sqx) - 1
        return (0.625 + cheb_eval_mode(bip2_cs, z, 29, -1, 1)) / y
...
cdef double _airy_Bi(double x) nogil:
    ...
    if x < -1:
        ...
        z = x ** 3
        mod = cheb_eval_mode(bif_cs, z, 9, -1, 1)
        theta = cheb_eval_mode(big_cs, z, 8, -1, 1)
        return 0.625 + mod + x * (0.4375 + theta)
    elif x <= 2:
        z = (2. * x ** 3 - 9) / 7
        mod = cheb_eval_mode(bif2_cs, z, 10, -1, 1)
        theta = cheb_eval_mode(big2_cs, z, 10, -1, 1)
